...
class DoujinShoppingListApp:
    def __init__(self, root):
        ...
        self.text_display.pack(pady=15, fill=tk.BOTH, expand=True)

        # フレーム作成
        self.frame_categories = tk.Frame(root)
        self.frame_categories.pack(pady=8, fill=tk.X)
        self.frame_quantity = tk.Frame(root)
        self.frame_quantity.pack(pady=8, fill=tk.X)
        self.frame_price = tk.Frame(root)
        self.frame_price.pack(pady=8, fill=tk.X)
        self.frame_numpad = tk.Frame(root)
        self.frame_numpad.pack(pady=8, fill=tk.X)
        self.frame_copy = tk.Frame(root)
        self.frame_copy.pack(pady=15, fill=tk.X)
        ...
        # コピー用ボタン
        copy_btn = tk.Button(self.frame_copy, text="コピー", font=("Arial", 14), command=self.copy_to_clipboard)
        copy_btn.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
